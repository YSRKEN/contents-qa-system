...
CLIとブラウザUIの手引き。設計の考え方は [README](../README.md)、
資料の集め方は [sourcing.md](sourcing.md) にあります。
...
`cqs serve` は手元のDBを直接書き換えますが、外出先の端末からは使えません。
ブラウザで開くだけの画面を用意しておき、そこで書き留めたものを後から取り込む道があります。

書き留められるのは3種類です。

| 種類 | 何を入れるか | 取り込むと |
|---|---|---|
| URL | あとで取得したいページ | `fetch` と同じく取得して原文層へ |
| 資料の本文 | 手元の資料の貼り付け | `add-text --kind manual` と同じ |
| メモ | 気付いたこと、確かめたいこと | 同上（見出しに「メモ」が付く） |

```bash
cqs -w 作品名 inbox --file 持ち帰り.json     # 画面の「まとめてコピー」で得たJSON
cqs -w 作品名 inbox --text '[{"kind":"url","body":"https://example.com/x"}]'
```

**入るのは原文層までです。** 主張にするかは通常どおり `propose` を通します。
出先で書いたものが、確認状態の付いた主張としてそのまま入ることはありません。
